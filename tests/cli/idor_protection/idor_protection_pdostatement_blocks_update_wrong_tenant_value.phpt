--TEST--
Test IDOR protection blocks PDOStatement::execute() UPDATE that filters on the wrong tenant ID value
...
$stmt = $pdo->prepare("UPDATE cats SET name = :name WHERE tenant_id = :tenant_id");
$stmt->execute([':name' => 'Rex', ':tenant_id' => 'org_456']);
