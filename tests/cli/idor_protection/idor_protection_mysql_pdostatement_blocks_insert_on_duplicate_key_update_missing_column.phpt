--TEST--
Test IDOR protection blocks a MySQL INSERT ... AS alias ON DUPLICATE KEY UPDATE upsert that omits the tenant column from VALUES
...
$upsert = $pdo->prepare("INSERT INTO cats (name) VALUES (?) AS vals ON DUPLICATE KEY UPDATE name = vals.name");
$upsert->execute(['Mittens']);
