--TEST--
Test pg_query_params() with a classic SQL injection payload in the SQL query
...
$result = pg_query_params($db, "SELECT * FROM (VALUES (1), (2)) AS users(id) WHERE id = " . $data['id'], []);
