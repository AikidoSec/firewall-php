...
func TestIsWebScanner(t *testing.T) {
	t.Run("Test is a web scanner", func(t *testing.T) {
		...
		assert.Equal(t, true, IsWebScanner("GET", "/", map[string]string{"test": "SELECT * FROM admin"}))
		assert.Equal(t, true, IsWebScanner("GET", "/", map[string]string{"test": "../etc/passwd"}))
	...
	t.Run("Test is not a web scanner", func(t *testing.T) {
		...
		assert.Equal(t, false, IsWebScanner("GET", "/", map[string]string{"test": "1'"}))
		assert.Equal(t, false, IsWebScanner("GET", "/", map[string]string{"test": "abcd"}))
	})
}
