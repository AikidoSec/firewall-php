...
ZEND_FUNCTION(should_block_request) {
    ...
    if (action.RetryAfter() > 0) {
        zend_update_property_long(blockingStatusClass, obj, "retry_after", sizeof("retry_after") - 1, action.RetryAfter());
    }
}
...
void RegisterAikidoBlockRequestStatusClass() {
    ...
    zend_declare_property_null(blockingStatusClass, "retry_after", sizeof("retry_after") - 1, ZEND_ACC_PUBLIC);
}
